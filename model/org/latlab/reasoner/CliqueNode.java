...
public class CliqueNode extends UndirectedNode {
	...
	LinkedHashSet<CliqueNode> getQualifiedNeiMsgs(){
		LinkedHashSet<CliqueNode> qualifiedNeiMsgs = new LinkedHashSet<CliqueNode>(_qualifiedNeiMsgs);
		return qualifiedNeiMsgs;
	}
	
	void setQualifiedNeiMsgs(Set<CliqueNode> qualifiedNeiMsgs) {
		_qualifiedNeiMsgs.clear();
		_qualifiedNeiMsgs.addAll(qualifiedNeiMsgs);
	}
	...
	LinkedList<Function> getFunction() {
		LinkedList<Function> func = new LinkedList<Function>();
		for(Function f : _funcs) {
			func.add(f.clone());
		}
		return func;
	}
	
	void setFunction(List<Function> func) {
		_funcs.clear();
		_funcs.addAll(func);
	}
	...
	Function getmsgsProd() {
		return _msgsProd.clone();
	}
	
	void setgetmsgsProd(Function msgsProd) {
		_msgsProd = msgsProd.clone();
	}
	...
	/**
	 * get all messages of this clique node
	 * @return messages of this clique node
	 * 
	 * @author fkhawar
	 */
	public Map<AbstractNode, Function> getMessages() {
		HashMap<AbstractNode, Function> msgCopy = new HashMap<AbstractNode, Function>();
		for(AbstractNode node : _msgs.keySet()) {
			Function copyFunction = _msgs.get(node).clone();
			msgCopy.put(node,copyFunction);
		}
		return msgCopy;
	}
	...
	public HashMap<AbstractNode, Double> getNormalization() {
		HashMap<AbstractNode, Double> alphasCopy = new HashMap<AbstractNode, Double>();
		alphasCopy.putAll(_alphas);
		return alphasCopy;
	}
	
	public HashMap<AbstractNode, Double> getLogNormalization() {
		HashMap<AbstractNode, Double> logAlphasCopy = new HashMap<AbstractNode, Double>();
		logAlphasCopy.putAll(logAlphas);
		return logAlphasCopy;
	}

	public void setLogNormalization(Map<AbstractNode, Double>  logAlpha) {
		logAlphas.clear();
		logAlphas.putAll(logAlpha);
	}
	
	public void setNormalization(HashMap<AbstractNode, Double>  alphas) {
		_alphas.clear();
		_alphas.putAll(alphas);
	}
	...
	/**
	 * <p>
	 * Replaces all the message this clique sends.
	 * </p>
	 * 
	 *  <p>
	 * <b>Note: Only methods of <code>CliqueTreePropagation</code> are supposed
	 ...
	 * @author fkhawar
	 */
	public void setMessage(Map<AbstractNode, Function>  messages) {
		_msgs.clear();
		_msgs.putAll(messages);
	}
