...
public class Utils {
	...
	public static double computeMutualInformation(Function dist) {
		...
		double[] cells1 = dist.sumOut(dist.getVariables().get(1))._cells; // child
		double[] cells2 = dist.sumOut(dist.getVariables().get(0))._cells; //parent

		// I(X;Y) = sum_X,Y P(X,Y) log P(X,Y)/P(X)P(Y)
		double mi = 0.0;
		int index = 0;
		for (double cell1 : cells1) { // child
			for (double cell2 : cells2) { // parent
	...
	public static <T> List<Map.Entry<T,Integer>> sortByAscendingOrderInt(Map<T, Integer> collection)
	{
        List<Map.Entry<T,Integer>> list = new ArrayList<Map.Entry<T,Integer>>(collection.entrySet());
	...
	public static <T> List<Map.Entry<T,Double>> sortByDescendingOrder(List<Map.Entry<T,Double>> list)
	{
