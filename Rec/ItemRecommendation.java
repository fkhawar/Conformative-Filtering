...
import org.mymedialite.itemrec.LTM_FR;
...
import org.mymedialite.eval.ItemsParallel.EvaluationTask;
...
public class ItemRecommendation {
  ...
  static String candidate_item_per_user_leave_one_evaluation_file=null;
  ...
  static int max_test_users=Integer.MAX_VALUE;
  ...
  public static void main(String[] args) {
    ...
    for(String arg : args) {
      ...
      else if(name.equals("--leave-one-file=")) 	  candidate_item_per_user_leave_one_evaluation_file       =value;
      ...
      else if(name.equals("--max-test-users="))       max_test_users       = Integer.parseInt(value);
    ...
    if(method.length()>8) substring = method.substring(0, 8); // Farhan. for explicit recommender, we will use the recommender name with the prefix explicit e.g. explicit ItemKNNCosine
    // Set up recommender
    if (load_model_file != null)
      try {
    	...
    	if(substring.equals("explicit")) // Farhan
    		recommender = Recommender.createRatingPredictor(method.substring(8)); // get the actual name of the recommender after removing hte explicit prefix
    	else
    ...
    if(method.equals("LTM_FR") ){
    	((LTM_FR)recommender).setItemMapping(item_mapping);
    	((LTM_FR)recommender).setUserMapping(user_mapping);
    	((LTM_FR)recommender).setComputeFit(compute_fit);

    	List<Integer> testUsers = new ArrayList<Integer>();
    	Collection<Integer> overlapItems = Utils.intersect(test_data.allItems(),
				training_data.allItems());
    	/*
    	 * Setting the test users
    	 */
		for (Integer user_id : test_users) {
			...
			HashSet<Integer> correct_items = new HashSet<Integer>(
					Utils.intersect(test_data.userMatrix().get(user_id),
							overlapItems));
			...
			int num_eval_items = overlapItems.size()
					- (candidate_items_in_train.size());
			...
			if (num_eval_items - correct_items.size() == 0)
				continue;
			testUsers.add(user_id);

		}
    	((LTM_FR)recommender).setTestUsers(testUsers);
    	System.out.println("Actual number of test users: "+ testUsers.size()+" Actual number of overlap test items: "+ overlapItems.size());

    	((LTM_FR)recommender).setCandidateItems(overlapItems);
      ...
      if (find_iter != 0) {
        ...
        if (load_model_file == null) {
          if (cross_validation > 1) {
            ...
            long start = Calendar.getInstance().getTimeInMillis();
            recommender.train();
            train_time = (double)(Calendar.getInstance().getTimeInMillis() - start) / 1000;
        ...
        if (prediction_file != null) {
          ...
          if (online_eval) {
            ...
            long start = Calendar.getInstance().getTimeInMillis();
            ItemRecommendationEvaluationResults res = evaluate();
            System.out.println(res);
            //ItemRecommenderEvaluate.recordMetrics(recommender.toString(), res, train_time+ "seconds", (double)(Calendar.getInstance().getTimeInMillis() - start) / 1000 + " seconds");
  ...
  static void loadData() throws Exception {
    ...
    // check for explicit recommender , if so then set the training data./ Farhan
    if (substring.equals("explicit")){

    	// reading below just for stats : Farhan

        training_data = ItemData.read(training_file, user_mapping, item_mapping, file_format == ItemDataFileFormat.IGNORE_FIRST_LINE);

        IRatings rat;
        if(file_format== ItemDataFileFormat.MovielensTime){
        	rat = MovieLensRatingData.read(Utils.combine(data_dir, training_file), user_mapping, item_mapping);
        	((TimeAwareRatingPredictor)recommender).setTimedRatings((ITimedRatings)rat);
        }
        else{
        //actual reading of data and setting it for explicit feedback
        	rat = RatingData.read(Utils.combine(data_dir, training_file), user_mapping, item_mapping, true);
        }
        ((RatingPredictor)recommender).setRatings(rat);


    }
    else{ // Farhan :enveloped the original training data loading in this else
    // training data
    ...
    // Test data. Placed here by Farhan
    if (test_ratio == 0) {
      if (test_file != null) {
  ...
  static ItemRecommendationEvaluationResults computeFit() throws Exception {
    if (filtered_eval)
      return ItemsFiltered.evaluateFiltered(recommender, training_data, training_data, item_attributes, test_users, candidate_items, true);
        ...
        // Farhan
    	if(candidate_item_per_user_leave_one_evaluation_file == null)
    		return ItemsParallel.evaluate(recommender, training_data, training_data, test_users, candidate_items,null, eval_item_mode, true,null,null,max_test_users,null);
    	//return Items.evaluate(recommender, test_data, training_data, test_users, candidate_items, eval_item_mode, repeat_eval);
    	else {
    		Map<Integer,List<Integer>> getCandidateItemPerUser = Extensions.getCandidateItemPerUser(user_mapping , item_mapping , candidate_item_per_user_leave_one_evaluation_file);
    		return ItemsParallel.evaluate(recommender, training_data, training_data, test_users, candidate_items,null, eval_item_mode, true,null,null,max_test_users,getCandidateItemPerUser);
    	}
    }
  }
  ...
  static ItemRecommendationEvaluationResults evaluate() throws Exception {
    if (filtered_eval)
      return ItemsFiltered.evaluateFiltered(recommender, test_data, training_data, item_attributes, test_users, candidate_items, repeat_eval);
        ...
        // Farhan
    	if(candidate_item_per_user_leave_one_evaluation_file == null)
    		return ItemsParallel.evaluate(recommender, test_data, training_data, test_users, candidate_items,null, eval_item_mode, repeat_eval,null,null,max_test_users,null);
    	//return Items.evaluate(recommender, test_data, training_data, test_users, candidate_items, eval_item_mode, repeat_eval);
    	else {
    		Map<Integer,List<Integer>> getCandidateItemPerUser = Extensions.getCandidateItemPerUser(user_mapping , item_mapping , candidate_item_per_user_leave_one_evaluation_file);
    		return ItemsParallel.evaluate(recommender, test_data, training_data, test_users, candidate_items,null, eval_item_mode, repeat_eval,null,null,max_test_users,getCandidateItemPerUser);
    	}
    }
