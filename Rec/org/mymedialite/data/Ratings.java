...
public class Ratings extends DataSet implements IRatings {
  ...
  public void buildByUserCounts() {
    countByUser = new IntArrayList(getMaxUserID() + 1);
    countByUser.size(getMaxUserID() + 1);
  ...
  public IntList countByItem() {
    if (countByItem == null || countByItem.size() < getMaxItemID() + 1)
      buildByItemCounts();
    return countByItem;
  }

  public void buildByItemCounts() {
    countByItem = new IntArrayList(getMaxItemID() + 1);
    for (int index = 0; index < size(); index++) {
  ...
  public double get(int user_id, int item_id, IntCollection indexes) {
    for (int index : indexes)
      if (users.getInt(index) == user_id && items.getInt(index) == item_id)
        return values.getDouble(index);
    throw new InvalidKeyException("rating " + user_id + ", " + item_id +  " not found." + " Index size:"+indexes.size());
  }
  ...
  public void add(int user_id, int item_id, double rating) {
    ...
    if (item_id > getMaxItemID())
      setMaxItemID(item_id);
    if (rating < minRating)
      minRating = rating;
    if (rating > maxRating)
      maxRating = rating;
  ...
  public void removeUser(int user_id) {
    ...
    if (getMaxUserID() == user_id)
      setMaxUserID(getMaxUserID() - 1);
  }
  ...
  public void removeItem(int item_id) {
    ...
    if (getMaxItemID() == item_id)
      setMaxItemID(getMaxItemID() - 1);
  }       
