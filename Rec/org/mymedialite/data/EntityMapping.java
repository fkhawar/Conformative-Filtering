...
public class EntityMapping implements IEntityMapping {
   ...
   */
  public HashMap<String, Integer> original_to_internal = new HashMap<String, Integer>();

  /**
   * Contains the mapping from the internal IDs to the original (external) IDs.
   */
  public HashMap<Integer, String> internal_to_original = new HashMap<Integer, String>();
  ...
  public void saveMappingInternalToOriginal(PrintWriter writer) throws IOException {
	    //writer.println(internal_to_original.size());
	    for(Entry<Integer, String> entry : internal_to_original.entrySet()) {
	      writer.println(entry.getKey() + " " + entry.getValue());
	      if(writer.checkError()) throw new IOException("Error writing model file");
	    }
	  }
