...
import org.mymedialite.datatype.SparseBooleanMatrixBinarySearch;
...
public class PosOnlyFeedback<T extends IBooleanMatrix> extends DataSet implements IPosOnlyFeedback {
  ...
  public void add(int user_id, int item_id) {
    ...
    if (user_id > getMaxUserID()) setMaxUserID(user_id);
    if (item_id > getMaxItemID()) setMaxItemID(item_id);
  }
  ...
  public void relase(){
	  for (int id=0 ; id <= getMaxUserID() ; id++){
		  
		  if( ((SparseBooleanMatrix)this.userMatrix).row_list.size()>0 ){
			  
			  ((SparseBooleanMatrix)this.userMatrix).row_list.get(id).clear();
			  
			  if (this.byUser != null){
				  this.byUser.get(id).clear(); 
			  }
		  }
	  }
	  ...
	  for (int id=0 ; id < maxItemID() ; id++){
		  if (this.byItem != null){
			  this.byItem.get(id).clear();
		  }
		  
	  }
	  
	  ((SparseBooleanMatrix)this.userMatrix).row_list.clear();
	  //((SparseBooleanMatrix)this.userMatrix).row_list = null;
  ...
  public void relase(Class<T> c){
	  for (int id=0 ; id <= getMaxUserID() ; id++){
		  
		  if( ((SparseBooleanMatrixBinarySearch)this.userMatrix).row_list.size()>0 ){
			  
			  ((SparseBooleanMatrixBinarySearch)this.userMatrix).row_list.get(id).clear();
	  ...
	  ((SparseBooleanMatrixBinarySearch)this.userMatrix).row_list.clear();
	  //((SparseBooleanMatrix)this.userMatrix).row_list = null;
