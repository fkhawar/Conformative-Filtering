    ...
    for (Entry<String, String> entry : parameters.entrySet()) {
      ...
      for (Field field : recommender.getClass().getFields()) {
        if (field.getName().equalsIgnoreCase(key)) {
          ...
          }else if (field.getType().getName().equals("java.lang.String")) { // Farhan
            field.set(recommender, entry.getValue());
          }  
          else {
            errorHandler.reportError("Parameter " + key + " has unknown type " + field.getType().getName());  
          }
        }
      }
    }
